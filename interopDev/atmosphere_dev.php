 ...
 * Si l'application tourne en local (127.0.0.1 ou ::1), on simule une IP
 * publique de Nancy pour tester le fonctionnement des APIs
 */
$client_ip = $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1';

if ($client_ip === '127.0.0.1' || $client_ip === '::1') {
    echo "<p class='info'>ℹ️ Mode local détecté, utilisation d'une IP de test pour la démonstration</p>\n";
    $client_ip = '90.126.179.10';
}

echo "<p class='info'>🌐 Adresse IP du client : {$client_ip}</p>\n";

// Appel de l'API ip-api.com pour géolocaliser l'adresse IP
$geoloc = geolocateIP($client_ip);

if (isset($geoloc['error'])) {
    echo "<p class='error'>⚠️ Erreur de géolocalisation : " . htmlspecialchars($geoloc['error']) . "</p>\n";
    // En cas d'échec de géolocalisation, utiliser les coordonnées de Nancy par défaut
    $latitude = 48.6937;
    $longitude = 6.1834;
    $location_name = "Nancy (coordonnées par défaut)";
} else {
    $latitude = $geoloc['latitude'];
    $longitude = $geoloc['longitude'];
    $location_name = $geoloc['city'] . ", " . $geoloc['region'];

    echo "<p class='info'>📍 Localisation détectée : {$location_name}</p>\n";

    // Si l'utilisateur n'est pas à Nancy, on récupère les coordonnées de l'IUT
    $is_nancy = (stripos($geoloc['city'], 'Nancy') !== false);

    if (!$is_nancy) {
        echo "<p class='info'>⚠️ Vous n'êtes pas à Nancy. Récupération des coordonnées de l'IUT Charlemagne...</p>\n";

        // Appel de Nominatim OSM pour géocoder l'adresse de l'IUT
        $iut = geocodeAddress("IUT Charlemagne Nancy France");

        if (isset($iut['error'])) {
            echo "<p class='info'>⚠️ Géocodage échoué, utilisation des coordonnées connues de l'IUT</p>\n";
            ...
            $location_name = "IUT Nancy-Charlemagne";
            echo "<p class='info'>✅ Coordonnées IUT récupérées via API : {$latitude}, {$longitude}</p>\n";
        }
    }
}
    ...
    <main>
        ...
        <section id="meteo">
            ...
            <div id="meteo-content">
                ...
                echo "<p class='info'>🌤️ Météo pour : <strong>{$location_name}</strong> (lat: {$latitude}, lon: {$longitude})</p>";
        ...
        <section id="trafic">
            ...
            if (isset($traffic['error'])) {
                echo "<p class='error'>⚠️ Erreur trafic : " . htmlspecialchars($traffic['error']) . "</p>\n";
                $traffic_json = json_encode(['incidents' => []]);
            } else {
                echo "<p class='info'>📊 {$traffic['count']} incident(s) de circulation détecté(s) dans le Grand Nancy</p>\n";
        ...
        <section id="covid">
            ...
            if (isset($covid['error'])) {
                echo "<p class='error'>⚠️ Erreur données SRAS : " . htmlspecialchars($covid['error']) . "</p>\n";
                $covid_json = json_encode(['weeks' => [], 'values' => []]);
            } else {
                $trend_icon = '➡️';
                $trend_class = 'stable';
                if ($covid['trend'] === 'hausse') {
                    $trend_icon = '↗️';
                    $trend_class = 'warning';
                } elseif ($covid['trend'] === 'baisse') {
                    $trend_icon = '↘️';
                    $trend_class = 'success';
                }

                echo "<p class='info'>📍 Station de mesure : <strong>Maxeville</strong> (Grand Nancy)</p>\n";
                echo "<p class='info'>📊 Dernière mesure (<strong>{$covid['latest_week']}</strong>) : ";
                echo "<span class='{$trend_class}'><strong>" . number_format($covid['latest_value'], 0, ',', ' ') . "</strong> {$covid['unit']} {$trend_icon}</span></p>\n";
        ...
        <section id="air-quality">
            ...
            if (isset($air['error'])) {
                echo "<p class='error'>⚠️ Erreur qualité de l'air : " . htmlspecialchars($air['error']) . "</p>\n";
            } else {
